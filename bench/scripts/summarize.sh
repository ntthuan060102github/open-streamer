...
restart_delta() {
  awk -F, 'NR==2 {first=$12} {last=$12} END {printf "%d", (last+0)-(first+0)}' "$CSV"
}

procs_max() {
  awk -F, 'NR>1 && $5>m {m=$5} END {printf "%d", m+0}' "$CSV"
}
...
read RSSP_AVG RSSP_MED RSSP_P95 RSSP_MAX <<<"$(stat_col "$CSV" 4)"
read GPU_AVG GPU_MED GPU_P95 GPU_MAX <<<"$(stat_col "$CSV" 6)"
read ENC_AVG ENC_MED ENC_P95 ENC_MAX <<<"$(stat_col "$CSV" 7)"
read DEC_AVG DEC_MED DEC_P95 DEC_MAX <<<"$(stat_col "$CSV" 8)"
read VRAM_AVG VRAM_MED VRAM_P95 VRAM_MAX <<<"$(stat_col "$CSV" 9)"
read RX_AVG RX_MED RX_P95 RX_MAX <<<"$(stat_col "$CSV" 10)"
read TX_AVG TX_MED TX_P95 TX_MAX <<<"$(stat_col "$CSV" 11)"
...
| RSS sum (% host) | $RSSP_AVG | $RSSP_MED | $RSSP_P95 | $RSSP_MAX |
...
\`\`\`markdown
| $RUN | $STREAM_COUNT | $LADDER | $CPU_MED | $RSS_MED | $RSSP_MED | $GPU_MED | $ENC_MED | $DEC_MED | $VRAM_MED | $RESTART_DELTA | $VERDICT |
\`\`\`
